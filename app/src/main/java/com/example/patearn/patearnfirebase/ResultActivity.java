...
public class ResultActivity extends AppCompatActivity {
    ...
    class BoardRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
            ...
            if (imageDTOs.get(position).stars.containsKey(mAuth.getCurrentUser().getUid())) {
        ...
        private void onStarClicked(DatabaseReference postRef) {
            postRef.runTransaction(new Transaction.Handler() {
                ...
                public Transaction.Result doTransaction(MutableData mutableData) {
                    ...
                    if (imageDTO.stars.containsKey(mAuth.getCurrentUser().getUid())) {
                        // Unstar the post and remove self from stars
                        imageDTO.starCount = imageDTO.starCount - 1;
                        imageDTO.stars.remove(mAuth.getCurrentUser().getUid());
                    } else {
                        // Star the post and add self to stars
                        imageDTO.starCount = imageDTO.starCount + 1;
                        imageDTO.stars.put(mAuth.getCurrentUser().getUid(), true);
                    }
            ...
            storage.getReference().child("images").child(imageDTOs.get(position).imageName).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                ...
                public void onSuccess(Void aVoid) {
                    ...
                    database.getReference().child("images").child(uidLists.get(position)).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            Toast.makeText(ResultActivity.this, "삭제가 완료되었습니다.", Toast.LENGTH_SHORT).show();
                        }
                ...
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(ResultActivity.this, "삭제 실패", Toast.LENGTH_SHORT).show();
                }
            });
        ...
        private class CustomViewHolder extends RecyclerView.ViewHolder {
            ...
            public CustomViewHolder(View view) {
                ...
                textView2 = view.findViewById(R.id.result_textView2);
                starButton = view.findViewById(R.id.result_starButton_imageView);
                deleteButton = view.findViewById(R.id.result_deleteButton);
            }
        }
    }
}
